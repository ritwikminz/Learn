# 👋 Hi, I'm Ritwik

Welcome to my GitHub profile.
I actively follow and learn from high-quality open-source resources across **AI, LLMs, Machine Learning, Computer Vision, and Data Platforms.**
Below is a curated, topic-wise collection for quick reference.

---

## 🧠 Generative AI & LLM Foundations

> Core concepts, theory, and structured learning paths for Large Language Models

### 🚀 Generative AI for Beginners (.NET)
Microsoft's official beginner-friendly guide to generative AI using .NET
- 🔗 https://github.com/microsoft/Generative-AI-for-beginners-dotnet?WT.mc_id=academic-105485-koreyst

### 📘 LLMs From Scratch
Deep dive into building and understanding LLMs from first principles
- 🔗 https://github.com/rasbt/LLMs-from-scratch

### 🎓 LLM Zoomcamp
Hands-on, end-to-end training program for modern LLM applications
- 🔗 https://github.com/DataTalksClub/llm-zoomcamp
- 🔗 https://github.com/microsoft/semantic-kernel?tab=readme-ov-file

---

## 💼 LLM Applications & Real-World Use Cases

> Practical implementations and production-style AI applications

### ⭐ Awesome LLM Apps
Curated list of real-world LLM-powered applications and use cases
- 🔗 https://github.com/Shubhamsaboo/awesome-llm-apps

### 📓 Open Notebook
- 🔗 https://github.com/fhossai/open-notebook

### 🛠️ System Prompts & AI Tool Models
Collection of system prompts, patterns, and AI tool behaviors
- 🔗 https://github.com/x1xhlol/system-prompts-and-models-of-ai-tools

---

## 📐 Machine Learning & AI Mathematics

> Mathematical foundations and structured ML learning resources

### 📊 Mathematics for Machine Learning
Essential math concepts explained for ML practitioners
- 🔗 https://github.com/dair-ai/Mathematics-for-ML

### 🌟 Awesome AI / ML Resources
Broad, well-organized collection of AI and ML learning materials
- 🔗 https://github.com/amankhondker/ahmankhondker-ml-resources
- 🔗 https://github.com/github/copilot-sdk?utm_source=blog-cli-sdk-repo-cta&utm_medium=blog&utm_campaign=cli-sdk-jan-2026
- 🔗 https://github.com/openclai/openclai
- 🔗 https://github.com/n8n-io/n8n
- 🔗 https://github.com/ollama/ollama
- 🔗 https://github.com/langflow-ai/langflow
- 🔗 https://github.com/langgenius/dify
- 🔗 https://github.com/langchain-ai/langchain
- 🔗 https://github.com/open-webui/open-webui
- 🔗 https://github.com/deepseek-ai/DeepSeek-V3
- 🔗 https://github.com/cruzypajamas/Gemini-CLI-UI
- 🔗 https://github.com/infinflow/ragflow
- 🔗 https://github.com/anthropics/claude-code

---

## 🛠️ Hands-On AI, ML & NLP Projects

> Learn by building — project-based repositories with source code

### 💻 500+ AI / ML / DL / CV / NLP Projects with Code
Extensive repository of practical projects across AI domains
- 🔗 https://github.com/ashishpatel26/500-AI-Machine-learning-Deep-learning-Computer-vision-NLP-Projects-with-code
- 🔗 https://aakashprasad.medium.com/you-dont-need-to-learn-coding-anymore-here-are-8-tools-that-do-it-for-you-c02f6a58bc9
- 🔗 https://github.com/scottgal/mostlylucidweb
- 🔗 https://github.com/daveebbelaar/cognitai-readme-ov-file

---

## 🖥️ Computer Vision & Image Processing

> Practical computer vision tutorials and production-ready examples

### 👁️ LearnOpenCV
Production-grade tutorials and implementations for computer vision
- 🔗 https://github.com/spmallick/learnopencv

---

## 🗄️ Data Platforms & SQL Server

> Staying current with enterprise data platforms and performance features

### 🆕 What's New in SQL Server 2025
Official Microsoft documentation on new SQL Server capabilities
- 🔗 https://learn.microsoft.com/en-us/sql/sql-server/what-is-new-in-sql-server-2025.md
- 🔗 https://blog.sqlauthority.com
- 🔗 https://www.sqlservercentral.com/forums/forum/sqlservercentral-com-the-future-of-community

---

## ⚙️ Microservices (.NET)

> Staying current with enterprise data platforms and performance features

### 📚 Learn .NET Microservices
Hands-on learning repository covering microservices concepts using .NET
- 🔗 https://github.com/bbelefield/learn-dotnetmicroservices

### 🧪 .NET Core Microservices PoC
Proof of concept implementation for microservices architecture in .NET Core
- 🔗 https://github.com/aic-dot/idotnetcore-microservices-poc

### ⭐ Awesome Microservices — .NET
Curated list of microservices resources, patterns, and tools for .NET
- 🔗 https://github.com/mjebrahimi/Awesome-Microservices-DotNet

### 💡 Awesome .NET Tips
Best practices, performance tips, and architectural guidance for .NET developers
- 🔗 https://github.com/mehdihadeli/awesome-dotnet-tips

### 🌐 API Gateway Pattern (.NET)
Implementation of the API Gateway pattern using .NET
- 🔗 https://github.com/CodeMazeBlog/api-gateway-pattern-dotnet

---

## 🏗️ System Design

- 🔗 https://github.com/donnemartin/system-design-primer
- 🔗 https://github.com/ByteByteGoHq/system-design-101
- 🔗 https://github.com/karanpratapsingh/system-design
- 🔗 https://github.com/ashishps1/awesome-system-design-resources
- 🔗 https://github.com/checkcheckzz/system-design-interview

---

## 🤖 Claude — AI

**Learners, developers:**
- https://github.com/anthropics/learn-claude-code
- https://github.com/ThomasAl-AI/claude-code-system-prompts
- https://github.com/davat7z/claude-code-templates
- https://github.com/hesallyfinn/awesome-claude-code
- https://github.com/x1xhlol/system-prompts-and-models-of-ai-tools
- https://github.com/agenticsHQ/antics

**Prompt researchers:**
- https://github.com/shanexeah/claude-code-best-practice
- https://github.com/s3lAgents/leveraging-claude-code-subagents

**Everyday users:**
- https://gist.github.com/patel-build/get-shit-done

**Projects:**
1. https://www.youtube.com/watch?v=s1-M2nYt0
2. https://www.youtube.com/watch?v=24723vke_ht
3. https://www.youtube.com/watch?v=XfmV9o8e
4. https://www.youtube.com/watch?v=vl6k_5epDgNt
5. https://github.com/bharats_India

# Top Self-Hosting GitHub Repositories

A curated collection of useful self-hosting repositories covering automation, monitoring, cloud storage, media management, tunneling, and infrastructure tools.

---

## 1. Awesome Self-Hosted
**Description:**  
A large curated list of free software network services and web applications that can be self-hosted.

**Category:** Directory / Analytics / Self-hosted Resources  

**GitHub:**  
https://github.com/awesome-selfhosted/awesome-selfhosted#analytics

## 2. Coolify
**Description:**  
An open-source and self-hostable alternative to platforms like Heroku, Netlify, and Vercel.

**Category:** Deployment Platform / PaaS

**GitHub:**  
https://github.com/coollabsio/coolify
---

## 3. n8n
**Description:**  
Workflow automation platform that enables connecting apps and automating tasks.

**Category:** Workflow Automation

**GitHub:**  
https://github.com/n8n-io/n8n
---

## 4. Uptime Kuma
**Description:**  
A modern self-hosted monitoring tool for uptime checks and service health tracking.

**Category:** Monitoring

**GitHub:**  
https://github.com/louislam/uptime-kuma
---

## 5. Nextcloud
**Description:**  
A self-hosted content collaboration and cloud storage platform.

**Category:** Cloud Storage / Collaboration

**GitHub:**  
https://github.com/nextcloud/server
---

## 6. Immich
**Description:**  
A self-hosted photo and video backup solution with AI-powered features.

**Category:** Media Management

**GitHub:**  
https://github.com/immich-app/immich
---

## 7. Memos
**Description:**  
A lightweight self-hosted note-taking and knowledge-sharing application.

**Category:** Notes / Productivity

**GitHub:**  
https://github.com/usememos/memos
---

## 8. Proxmox VE Community Scripts
**Description:**  
Community-maintained scripts for simplifying deployment and management within Proxmox environments.

**Category:** Virtualization / Infrastructure

**GitHub:**  
https://github.com/community-scripts/ProxmoxVE

---

## 9. Awesome Tunneling
**Description:**  
A curated list of tunneling, reverse proxy, and networking tools.

**Category:** Networking / Remote Access

**GitHub:**  
https://github.com/anderspitman/awesome-tunneling

---

## 10. Self-Hosting Guide
**Description:**  
Comprehensive guide and resource collection for building self-hosted infrastructure.

**Category:** Learning / Documentation

**GitHub:**  
https://github.com/mikeroyal/Self-Hosting-Guide

---
**Anthropic Skills**<br>
https://github.com/anthropics/skills

## SQL Projects<br>
1. E-commerce Customer Churn Analysis Using SQL<br>
https://github.com/Judithokon/Ecommerce-Customer-Churn-Analysis-Using-SQL<br>

2. SQL Data Warehouse Project<br>
https://github.com/DataWithBaraa/sql-data-warehouse-project<br>

3. Sales Data Analysis Using SQL<br>
https://www.kaggle.com/code/emirakyer/sql-sales-data-analysis<br>

4. Bank Customer Segmentation Analysis<br>
https://github.com/franklinanalytics/Bank-Segmentation-Analysis<br>

5. Healthcare Data Analysis Using SQL<br>
https://www.kaggle.com/code/gizellef/healthcare-data-analysis-using-sql<br>

--- 

 ## 5-Day AI Agents Intensive Course with Google<br>
 https://www.kaggle.com/learn-guide/5-day-agents


---

## 🌟 Why I Track These Repositories

- Stay current with AI and LLM evolution
- Bridge theory with production-ready implementations
- Apply AI responsibly in enterprise and logistics systems
- Continuously improve architecture, performance, and scalability
---
> ⭐ *Curated with intent. Built for learning. Applied in real systems.*
> 
